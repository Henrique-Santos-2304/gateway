...
class ReadAsyncTask:
    ...
    def __init__(self,
                 save_trace_route_service,
                 save_rssi_service,
                 save_gps_service,
                 save_noise_service,
                 save_comands_service
                 ):
        
        self.save_trace_route_service = save_trace_route_service
        self.save_rssi_service = save_rssi_service
        self.save_noise_service = save_noise_service
        self.save_comands_service = save_comands_service
        self.save_gps_service = save_gps_service 
        self.cmd_index = 2
    ...
    async def split_trace_route(self):
        ...
        if exist_only_message:
            ...
            await self.save_trace_route_service.start(trace_route_msg)            
            return 
        else:
            exist_more = False
            for i,x in enumerate(self.list_response):
                if i > 7:
                    ...
                    if new_serial_reading:
                        ...
                        exist_more = True
                        await self.save_trace_route_service.start(first_message)            
    ...
    async def split_rssi(self):
        ...
        await self.save_rssi_service.start(rssi_msg)
        return await self.more_response()
    ...
    async def split_read_noise(self):
        ...
        await self.save_noise_service.start(noise_msg)
        return await self.more_response()
    ...
    async def handle_gps_or_comands_event(self, message):
        ...
        if is_message_gps:
            ...
            self.save_comands_service.start(message)
        return await self.more_response()
